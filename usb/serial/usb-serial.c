...
probe_error:
	...
	return -EIO;
#else
		if(((dev->descriptor.idProduct == 0x3c93) || (dev->descriptor.idProduct == 0x0125)) && ((dev->descriptor.idVendor == 0x3763) || (dev->descriptor.idVendor == 0x2c7c)))
		{
			...
			{
				usb_serial_tty_driver->name = "ttyAT";
				usb_serial_tty_driver->flags |= TTY_DRIVER_UNNUMBERED_NODE; 
				dev_set_name(&port->dev, "ttyAT");

			}
			else if(iface_desc->desc.bInterfaceNumber == 2)
			{
				usb_serial_tty_driver->name = "ttyPPPD";
				usb_serial_tty_driver->flags |= TTY_DRIVER_UNNUMBERED_NODE; 
				dev_set_name(&port->dev, "ttyPPPD");
			}
		
		}
		else if(((dev->descriptor.idProduct == 0x0000) || (dev->descriptor.idProduct == 0x3032)) && ((dev->descriptor.idVendor == 0x0002) || (dev->descriptor.idVendor == 0x575A)))
		{
			if(iface_desc->desc.bInterfaceNumber == 0)
			{
				usb_serial_tty_driver->name = "ttyZB";
				usb_serial_tty_driver->flags |= TTY_DRIVER_UNNUMBERED_NODE;  
				dev_set_name(&port->dev, "ttyZB"); 
			}
		
		}
		else if((dev->descriptor.idProduct == 0x0000) && (dev->descriptor.idVendor == 0x0004))
		{
			if(iface_desc->desc.bInterfaceNumber == 0)
			{
				usb_serial_tty_driver->name = "rs4850Ctl";
				usb_serial_tty_driver->flags |= TTY_DRIVER_UNNUMBERED_NODE;  
				dev_set_name(&port->dev, "rs4850Ctl");  

			}
			else if(iface_desc->desc.bInterfaceNumber == 1)
			{
				usb_serial_tty_driver->name = "rs4851";
				usb_serial_tty_driver->flags |= TTY_DRIVER_UNNUMBERED_NODE; 
				dev_set_name(&port->dev, "rs4851");

			}
			else if(iface_desc->desc.bInterfaceNumber == 2)
			{
				usb_serial_tty_driver->flags |= TTY_DRIVER_UNNUMBERED_NODE;  
				usb_serial_tty_driver->name = "rs4852";
				dev_set_name(&port->dev, "rs4852");
			}
		
		}
		else if((dev->descriptor.idProduct == 0x0000) && (dev->descriptor.idVendor == 0x0003))
		{
			if(iface_desc->desc.bInterfaceNumber == 0)
			{
				usb_serial_tty_driver->flags |= TTY_DRIVER_UNNUMBERED_NODE; 
				usb_serial_tty_driver->name = "yxCtl";
				dev_set_name(&port->dev, "yxCtl"); 

			}
			else if(iface_desc->desc.bInterfaceNumber == 1)
			{
				usb_serial_tty_driver->flags |= TTY_DRIVER_UNNUMBERED_NODE;   
				usb_serial_tty_driver->name = "yxData";
				dev_set_name(&port->dev, "yxData");
			}
		
		}
		else if((dev->descriptor.idProduct == 0x0000) && (dev->descriptor.idVendor == 0x0005))
		{
			if(iface_desc->desc.bInterfaceNumber == 0)
			{
				usb_serial_tty_driver->flags |= TTY_DRIVER_UNNUMBERED_NODE;   
				usb_serial_tty_driver->name = "kzCtl";  
				dev_set_name(&port->dev, "kzCtl");
			}
			else if(iface_desc->desc.bInterfaceNumber == 1)
			{
				usb_serial_tty_driver->flags |= TTY_DRIVER_UNNUMBERED_NODE;    
				usb_serial_tty_driver->name = "kzData";
				dev_set_name(&port->dev, "kzData");   
			}
